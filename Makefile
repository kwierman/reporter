...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=mypackage tests/
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 mypackage tests
...
check-safety:
	poetry check
	poetry run bandit -ll --recursive mypackage tests
